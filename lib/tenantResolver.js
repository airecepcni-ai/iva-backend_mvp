...
export function extractCalledNumberDetailed(payload) {
  if (!payload || typeof payload !== 'object') return { calledNumber: null, sourcePath: null };
  ...
  // Priority order:
  // Prefer *business inbound* number for tenant resolution.
  // Only fall back to caller/customer numbers as a last resort.
  const candidates = [
    // === Vapi preferred fields (MUST be highest priority) ===
    ...
    { path: 'body.message.phoneNumber.number', value: body?.message?.phoneNumber?.number },
    { path: 'body.message.call.to', value: body?.message?.call?.to },
    { path: 'body.to', value: body?.to },

    // === Fallbacks (caller/customer) ===
    { path: 'body.message.call.customer.number', value: body?.message?.call?.customer?.number },
    { path: 'body.from', value: body?.from },
  ];

  for (const c of candidates) {
    if (typeof c.value === 'string' && c.value.trim().length > 0) {
      return { calledNumber: c.value.trim(), sourcePath: c.path };
    }
  }
  ...
  return legacy ? { calledNumber: legacy, sourcePath: 'legacy' } : { calledNumber: null, sourcePath: null };
}
